(() => {
  ...
  try {
    ...
    const player = {
      ...
      bulletLife: 1.0,
      bulletSize: 4,
      moveAngle: 0,     // old ship rotation (A/D)
    ...
    function step(now) {
      ...
      if (Math.hypot(player.vx, player.vy) < 10) {        player.vx = 0;
        player.vy = 0;
      }
      ...
      // ship angle:
      // - moving: point toward movement direction (if moving)
      // - stopped + aimLock: point at enemy (shoot direction)
      if (aimLock && tgt) {
      ...
      renderDebugOverlay(
        `Entities: enemies=${enemies.length} bullets=${bullets.length} ebullets=${ebullets.length}\n` +
