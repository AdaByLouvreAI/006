(() => {
  ...
  function renderLawBook(extra = "") {
    ...
    el.textContent =
      ...
      `Counts: spawns=${c.spawns} kills=${c.kills} buffs=${c.buffs} cardRolls=${c.cardRolls} hits=${c.hits} ricochet=${c.ricochets} boom=${c.boom} volleys=${c.volleys} resets=${c.resets} blocked=${c.spawnBlocked} fleetCapBlocks=${c.fleetCapBlocks}\n` +
      (extra ? extra + "\n" : "") +
      "— — — — — — — — — — — — — —\n" +
      LAW.lines.join("\n");
  }
  ...
  try {
    ...
    const SETTINGS = {
      ...
      // 06.0 §BUFFS (old random-buff roll OFF by default; card-roll system handles buffs)
      BUFF_RANDOM_ENABLED: false,
      ...
      // ✅ ENEMY HP MULTIPLIER (NERFED)
      ENEMY_HP_MULT: 25.0, // was 100.0
    ...
    // ============================================================
    // 06.5 — CARD ROLL BUFFS (NO DROPS, INSTANT APPLY)
    // ============================================================
    const CARD_ELEMS = [
      { name: "Heat", color: "#ff8a2a" },
      { name: "Cold", color: "#4aa3ff" },
      { name: "Toxin", color: "#34d058" },
      { name: "Electricity", color: "#b180ff" },
      { name: "Viral", color: "#7bffb0" },
      { name: "Corrosive", color: "#9aff3a" },
      { name: "Radiation", color: "#ffe066" },
      { name: "Magnetic", color: "#66fff0" },
      { name: "Blast", color: "#ff5c5c" },
    ];

    const CARD_RARITIES = [
      { tier: 1, name: "Common",   rollMax: 100,      negChance: 0.00 },
      { tier: 2, name: "Uncommon", rollMax: 1000,     negChance: 0.06 },
      { tier: 3, name: "Rare",     rollMax: 10000,    negChance: 0.12 },
      { tier: 4, name: "Prime",    rollMax: 100000,   negChance: 0.20 },
      { tier: 5, name: "Satanic",  rollMax: 1000000,  negChance: 0.32 },
    ];

    function rollCardRarity() {
      const roll = Math.floor(Math.random() * 1000000) + 1;
      for (const r of CARD_RARITIES) if (roll <= r.rollMax) return { ...r, roll };
      return { ...CARD_RARITIES[CARD_RARITIES.length - 1], roll };
    }
    function pickElement() {
      return CARD_ELEMS[Math.floor(Math.random() * CARD_ELEMS.length)];
    }
    function cardTraitCount(tier) {
      if (tier <= 2) return 1;
      if (tier === 3) return 2;
      if (tier === 4) return 3;
      return 4;
    }
    function scaleByTier(v, tier) {
      return v * (1 + (tier - 1) * 0.35);
    }
    function rollTraitValue(tr, tier, negative) {
      let v = rand(tr.min, tr.max);
      v = scaleByTier(v, tier);
      if (negative) v *= -rand(0.45, 0.95);

      // tidy
      if (Number.isInteger(tr.min) && Number.isInteger(tr.max)) return Math.round(v);
      return Math.round(v * 100) / 100;
    }

    const CARD_TRAITS = [
      // tank
      { label: "Max HP",    min: 10,  max: 34,  apply: (v) => { player.hpMax += v; player.hp = Math.min(player.hp + Math.max(0, v), player.hpMax); } },
      { label: "DR Flat",   min: 0.12,max: 0.55,apply: (v) => { player.drFlat += v; } },
      { label: "DR %",      min: 0.01,max: 0.04,apply: (v) => { player.drPct = clamp(player.drPct + v, 0, 0.85); } },
      { label: "Regen",     min: 0.08,max: 0.45,apply: (v) => { player.regen = Math.max(0, player.regen + v); } },

      // damage/shooting
      { label: "Damage",       min: 1,   max: 6,   apply: (v) => { player.baseDamage = Math.max(1, player.baseDamage + v); } },
      { label: "Fire Rate",    min: 0.3, max: 1.6, apply: (v) => { player.fireRate = Math.max(0.5, player.fireRate + v); } },
      { label: "Bullet Speed", min: 25,  max: 110, apply: (v) => { player.bulletSpeed = Math.max(180, player.bulletSpeed + v); } },
      { label: "Bullet Size",  min: 0.4, max: 2.0, apply: (v) => { player.bulletSize = clamp(player.bulletSize + v, 2, 18); } },
      { label: "Spread",       min: -0.03,max: 0.05, apply: (v) => { player.spreadBase = clamp(player.spreadBase + v, 0.04, 0.5); } },

      // movement
      { label: "Max Speed", min: 10,  max: 55,  apply: (v) => { player.maxSpeed = Math.max(120, player.maxSpeed + v); } },
      { label: "Thrust",    min: 60,  max: 220, apply: (v) => { player.thrust = Math.max(600, player.thrust + v); } },
      { label: "Drag",      min: -0.40,max: 0.30, apply: (v) => { player.drag = clamp(player.drag + v, 1.2, 10.0); } },

      // special
      { label: "Pierce",   min: 1, max: 1, apply: (v) => { player.pierce = Math.max(0, player.pierce + v); } },
      { label: "Ricochet", min: 1, max: 1, apply: (v) => { player.ricochet = Math.max(0, player.ricochet + v); } },
      { label: "Fleet +1", min: 1, max: 1, apply: (v) => { safeFleetAdd(v); } },

      // explosive mode
      { label: "Explosive Mode", min: 1, max: 1, apply: () => { player.explosive = true; player.exploRadius = Math.max(player.exploRadius, 80); } },
    ];

    function rollCardAndApply(trigger = "kill") {
      const rarity = rollCardRarity();
      const elem = pickElement();
      const n = cardTraitCount(rarity.tier);

      const picked = [];
      for (let i = 0; i < n; i++) {
        const tr = CARD_TRAITS[Math.floor(Math.random() * CARD_TRAITS.length)];
        const negative = Math.random() < rarity.negChance;
        const val = rollTraitValue(tr, rarity.tier, negative);
        picked.push({ tr, val });
      }

      for (const p of picked) p.tr.apply(p.val);

      syncFleet();

      const title = `CARD T${rarity.tier} ${rarity.name} [${elem.name}]`;
      const details = picked.map(p => `${p.tr.label}:${p.val}`).join(", ");
      buffs.push({ name: `${title} → ${details}`, apply() {} });

      LAW.counts.buffs++;
      LAW.counts.cardRolls++;
      law(6, "CARD", `${title} trigger=${trigger} roll=${rarity.roll} :: ${details}`);
    }
    ...
    // ============================================================
    // SPAWN ENEMY (HP multiplier nerfed in SETTINGS)
    // ============================================================
    function spawnEnemy() {
    ...
    function killEnemy(e, idxHint = -1) {
      ...
      // ✅ card-roll buffs apply instantly (NO DROPS)
      rollCardAndApply("kill");

      // optional: keep the old random roll system (OFF by default in SETTINGS)
    ...
    function step(now) {
      ...
      if (fireCooldown <= 0) {
        shoot();
        followersShoot();
        fireCooldown = period * (followers.length ? player.fleetFireMult : 1.0);
      }
